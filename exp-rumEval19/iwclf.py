...
class ImportanceWeightedClassifier(object):
    ...
    def iwe_ratio_gaussians(self, X, Z):
        ...
        if not (self.is_pos_def(Si_X) or self.is_pos_def(Si_Z)):
            print('Warning: covariate matrices not PSD.')

            regct = -6
            while not (self.is_pos_def(Si_X) or self.is_pos_def(Si_Z)):
    ...
    def iwe_kernel_mean_matching(self, X, Z,
        ...
        Z : array
            target data (M samples by D features)
        distance : str
            Which type of distance to use (def: 'euclidean')
        self_normalize : bool
            Whether to normalize the weights to sum to N.
        ...
        KXX = cdist(X, X, metric=distance)
        KXZ = cdist(X, Z, metric=distance)
